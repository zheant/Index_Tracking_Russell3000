...
// Undocumented, a wrong way often seen, supporting it won't hurt
...
GRBModel::sync()
{
  ...
  // If the optimization is still in progress, we do not remove the
  // callback, hence, we cannot remove the callback in the C++ world.
  if (error == GRB_ERROR_OPTIMIZATION_IN_PROGRESS)
    throw GRBException(string(GRBgeterrormsg(Cenv)), error);

  // Collect the error message before we remove the callback. Removing the
  // callback clears the error message.
  string errorMsg;
  if (error)
    errorMsg = string(GRBgeterrormsg(Cenv));

  if (cb != NULL) {
    int unseterror = GRBsetcallbackfunc(Cmodel, NULL, NULL);
    assert(unseterror == 0 || unseterror == error);
    (void) unseterror;
  }

  if (error) throw GRBException(errorMsg, error);
}
...
GRBModel::dualize()
{
  ...
  GRBModel xmodel = GRBModel();
  int error = GRBdualmodel(Cmodel, &xmodel.Cmodel);
  if (error) throw
    GRBException(string(GRBgeterrormsg(Cenv)), error);
  else if (xmodel.Cmodel == NULL) throw
    GRBException("Unable to create dualized model",
                 GRB_ERROR_FAILED_TO_CREATE_MODEL);
                     ...
                     const double*    rhspen)
{
  ...
  prefetchAttr(GRB_IntAttr_NumVars);
  prefetchAttr(GRB_IntAttr_NumConstrs);
  prefetchAttr(GRB_IntAttr_NumQConstrs);
  prefetchAttr(GRB_IntAttr_NumGenConstrs);
...
GRBModel::populate(bool emptyModel)
{
  ...
  if (emptyModel) {
    ...
    prefetchAttr(GRB_IntAttr_NumVars);
    prefetchAttr(GRB_IntAttr_NumConstrs);
    prefetchAttr(GRB_IntAttr_NumSOS);
    prefetchAttr(GRB_IntAttr_NumQConstrs);
    prefetchAttr(GRB_IntAttr_NumGenConstrs);
    ...
    vars.reserve(cols);
    constrs.reserve(rows);
    sos.reserve(numsos);
    qconstrs.reserve(numqconstrs);
    genconstrs.reserve(numgenconstrs);
  }
...
int
GRBModel::hasPendingChanges() const
{
  for (unsigned int i = 0; i < vars.size(); i++)
    if (vars[i].getcolno() <= -2)
      return 1;

  for (unsigned int i = 0; i < constrs.size(); i++)
    if ( constrs[i].getrowno() <= -2)
      return 1;
  ...
  for (unsigned int i = 0; i < qconstrs.size(); i++)
    if (qconstrs[i].getindex()  <= -2)
      return 1;

  for (unsigned int i = 0; i < genconstrs.size(); i++)
    if (genconstrs[i].getindex() <= -2)
      return 1;
...
GRBModel::optimize()
{
  ...
  // remove user callback before evaluation the error to ensure that the
  // callback is always removed
  if (cb != NULL) {
    int unseterror = GRBsetcallbackfunc(Cmodel, NULL, NULL);
    assert(unseterror == 0);
    (void) unseterror;
...
GRBModel::optimizeasync()
{
  ...
  if (error) {
    // Collect the error message before we remove the callback. Removing the
    // callback clears the error message.
    string errorMsg = string(GRBgeterrormsg(Cenv));

    // remove the callback in case of an error, otherwise we remove the
    // callback after a successful sync() call
    if (cb != NULL) {
      int unseterror = GRBsetcallbackfunc(Cmodel, NULL, NULL);
      assert(unseterror == 0);
      (void) unseterror;
    ...
    throw GRBException(errorMsg, error);
  }
}

void
GRBModel::convertToFixed()
{
  if (hasPendingChanges())
    throw GRBException("Error: model has pending changes, not supported for GRBconverttofixed()",
                       GRB_ERROR_NOT_SUPPORTED);

  int error = GRBconverttofixed(Cmodel);
  ...
  // The fixed models could have additional variables or constraints.
  // If so, we need to construct the corresponding C++ handles.
  int i, n;

  error = GRBgetintattr(Cmodel, "NumVars", &n);
  if (error) throw GRBException(string(GRBgeterrormsg(Cenv)), error);

  for (i = static_cast<int>(vars.size()); i < n; i++)
    vars.push_back(GRBVar(Cmodel, i));

  error = GRBgetintattr(Cmodel, "NumConstrs", &n);
  if (error) throw GRBException(string(GRBgeterrormsg(Cenv)), error);

  for (i = static_cast<int>(constrs.size()); i < n; i++)
    constrs.push_back(GRBConstr(Cmodel, i));

  error = GRBgetintattr(Cmodel, "NumQConstrs", &n);
  if (error) throw GRBException(string(GRBgeterrormsg(Cenv)), error);

  for (i = static_cast<int>(qconstrs.size()); i < n; i++)
    qconstrs.push_back(GRBQConstr(Cmodel, i));

  n = static_cast<int>(sos.size());
  for (i = 0; i < n; i++) {
    if (sos[i].sosRep)
      delete sos[i].sosRep;
  }
  ...
  n = static_cast<int>(genconstrs.size());
  for (i = 0; i < n; i++) {
    if (genconstrs[i].genconRep)
      delete genconstrs[i].genconRep;
  }
  genconstrs.clear();
}
                           ...
                           const char*   name)
{
  ...
  int error = GRBaddgenconstrNorm(Cmodel, name, resind, len, ind, which);
  delete[] ind;
  if (error) throw GRBException(string(GRBgeterrormsg(Cenv)), error);
                               ...
                               string      coptions)
{
  ...
  int error = GRBaddgenconstrLogistic(Cmodel, name, xind, yind, options);
  if (error) throw GRBException(string(GRBgeterrormsg(Cenv)), error);
...
GRBGenConstr
GRBModel::addGenConstrNL(GRBVar        yvar,
                         int           nnodes,
                         const int*    opcode,
                         const double* data,
                         const int*    parent,
                         string        cname)
{
  return addGenConstrNL(yvar, nnodes, opcode, data, parent, cname.c_str());
}
                         ...
                         const char*   name)
{
  ...
  int yind = yvar.getcolno();
  if (yind < 0) throw
    GRBException("yvar not in the model", GRB_ERROR_NOT_IN_MODEL);

  int error = GRBaddgenconstrNL(Cmodel, name, yind, nnodes, opcode, data, parent);
  if (error) throw GRBException(string(GRBgeterrormsg(Cenv)), error);
                           ...
                           double*       whichP)
{
  ...
  int error = GRBgetgenconstrNorm(Cmodel, genci, &resvar, &nvars, NULL, &which);
  if (error)
    throw GRBException(string(GRBgeterrormsg(Cenv)), error);

  if (xvars != NULL) {
    int* ind = new int[nvars];
    error = GRBgetgenconstrNorm(Cmodel, genci, NULL, NULL, ind, NULL);
    if (error) {
      delete[] ind;
      throw GRBException(string(GRBgeterrormsg(Cenv)), error);
    }
                               ...
                               GRBVar*      yvarP)
{
  ...
  int xind, yind;
  int error = GRBgetgenconstrLogistic(Cmodel, genci, &xind, &yind);
  if (error)
    throw GRBException(string(GRBgeterrormsg(Cenv)), error);
...
void
GRBModel::getGenConstrNL(GRBGenConstr       genc,
                         GRBVar*            yvarP,
                         int*               nnodesP,
                         int*               opcode,
                         double*            data,
                         int*               parent)
{
  ...
  int yvari;
  int error = GRBgetgenconstrNL(Cmodel, genci, &yvari, nnodesP,
                                opcode, data, parent);
  if (error)
    throw GRBException(string(GRBgeterrormsg(Cenv)), error);
  if(yvarP != NULL){
    GRBVar yvar = vars.at(yvari);
    yvari = yvar.getcolno();
    if (yvari < 0 || yvari >= cols)
      throw GRBException("Not in the model", GRB_ERROR_NOT_IN_MODEL);
    *yvarP = yvar;
  }
}
...
{
  // We must not change the user callback while we are executing an async()
  // call. For this reason, we should first test whether we are currently
  // inside an optimization. To do so, we just call GRBsetcallbackfunc() to
  // set the callback to NULL. If we are inside an optimization, this will
  // return an error, so that we can raise an exception. If it succeeds,
  // then this call was actually a no-op, because the internal C callback
  // function should always be NULL outside of an optimization.
  if (cb != NULL) {
