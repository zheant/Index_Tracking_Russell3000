...
GRBCallback::xcb(GRBmodel *xmodel, void *xcbdata, int xwhere, void *xuserdata)
{
  ...
  (void)xmodel; // avoid compiler warning
  cb->cbdata = xcbdata;
  cb->where  = xwhere;
  cb->callback();
  if (cb->newx != NULL)
...
void
GRBCallback::set(GRB_IntParam param, int newvalue)
{
  int error = GRBcbsetintparam(cbdata, iparname[param], newvalue);
  if (error) throw GRBException("set", error);
}

void
GRBCallback::set(GRB_DoubleParam param, double newvalue)
{
  int error = GRBcbsetdblparam(cbdata, dparname[param], newvalue);
  if (error) throw GRBException("set", error);
}

void
GRBCallback::set(GRB_StringParam param, const string& newvalue)
{
  int error = GRBcbsetstrparam(cbdata, sparname[param], newvalue.c_str());
  if (error) throw GRBException("set", error);
}
...
GRBCallback::proceed()
{
  int error = GRBcbproceed(cbdata);
  if (error) throw GRBException("proceed", error);
}
